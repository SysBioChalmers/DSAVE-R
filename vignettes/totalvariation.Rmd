...
library(ggplot2)
library(DSAVER)
data(tcellCD4ProfilesTMMNormalized, package = "DSAVER")
ub <- 100000
lb <- 0.5

bulkMean1Vs1 <- DSAVEGetTotalVariationFromBulk(tcellCD4ProfilesTMMNormalized, 
                    pool4samples = FALSE, upperBoundTPM = ub,
                    lowerBoundTPM = lb, rescale = FALSE)
bulkMean4Vs4 = DSAVEGetTotalVariationFromBulk(tcellCD4ProfilesTMMNormalized, 
                    pool4samples = TRUE, upperBoundTPM = ub, na.rm = TRUE, seed = 2019L, 
                    lowerBoundTPM = lb, nComb = 10000L, rescale =FALSE)
```
...
```{r}
data(PBMC68K_Tcells, package = "DSAVER")

scTotalVariation <- list()
pools <- seq(200, 4000, by = 400)

for(poolSize in pools){
  scTotalVariation[[as.character(poolSize)]] <- DSAVEGetTotalVariationPoolSize(PBMC68K_Tcells, 
        poolSize = poolSize, upperBoundTPM = 1e5, lowerBoundTPM = 5e-1, na.rm = TRUE, seed = NULL,
        repetitionPerSize = 30L, rescale = FALSE)
}
